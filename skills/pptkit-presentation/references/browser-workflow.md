...
2. Serialize the completed manifest to `deck-session.json`, then prepare it as a `session` payload with `application/json`, using the session ID as `payloadId`. `preparePptkitTransfer()` runs the self-contained session and layout-recipe preflight; do not send any chunk when it reports a schema or layout incompatibility.
