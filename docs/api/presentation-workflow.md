...
`SlidePlan.composition`, `SlidePlan.density`, and `SlidePlan.visualIntent` optionally carry the outline's design intent into authoring. Visual intent is one of `content-led`, `image-led`, `color-led`, `data-led`, or `type-led`. Images may be used by cover, section, statement, image, KPI, and closing roles; `image-background` and `color-field` are explicit immersive compositions. When intent is absent, `planDeckLayout()` selects a compatible recipe from the deck as a whole, using content shape, adjacent rhythm, theme, and `DeckSpec.design.seed`. The same spec and seed always produce the same `LayoutDecision[]`; incompatible explicit compositions or visual intents are validation errors rather than silent fallbacks.
...
- `auditVisualRhythm` reports per-slide image and deliberate color-field coverage, resolved visual intent, visual-anchor slides, and long low-intensity content runs. Browser and Node builds store this evidence in `BuildReport.visualAudit`.
